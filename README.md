...
- **目などの白飛び**：「目などの白飛びを抑える」は初期状態でOFFです。ONにした場合、ベース画像と発光画像に同じテクスチャが使われる場合に発光を省きます。通常はOFFのまま、意図した発光を保存します。
- **焼き込み後に目の色が変わる**：ワンクリック書き出しの「別画像の強い発光も抑える」は初期状態でOFFです。ONにした場合、焼き込み前の目画像など、別のテクスチャを使うHDR発光も省きます。目以外の意図的なHDR発光にも適用されるため、必要な場合は解除してください。
