...
**VRChatの表情 > 表情を取り込む**を有効にして、元のUnityアバターから
...
**ジェスチャーの表情も取り込む**は標準で有効です。FX Controllerで
`GestureLeft` / `GestureRight` の条件から遷移する状態に登録された、固定の顔の
AnimationClipを候補に追加します。アニメーション内の目・眉・口などのBlendShape値を
...
ありません。変化し続ける曲線、材質・表示・ボーンを同時に変更するクリップは省略します。
元の顔が複数レイヤーやDriverの合成で作られる場合は、その固定顔を一つにまとめた
表情アニメーションが必要です。ビルド時生成のControllerは引き続き対象外です。

確認画面には取り込む表情の件数を表示し、不要な候補は外せます。取り込めない項目の
詳細は折りたたんであります。完了ダイアログは保存結果とVRChat表情件数だけを表示し、
