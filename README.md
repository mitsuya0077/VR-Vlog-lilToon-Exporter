...
- **独立したパーツの追従先を確認する** — 頭などとの接続が確定していないパーツは、影響するメッシュを表示し、コピーを動かすプレビューから追従先を選べます。現在の接続を保って書き出すこともできます。
...
- **AutoAnchorObjectを使う小物の位置**：Root Boneの影響を受ける、ボーンウェイトのない頂点を明示的なウェイトへ変換します。Unityの描画用メッシュを変換前後で比較し、配置を保てない場合は保存前に理由を表示します。元のメッシュやRoot Boneは変更しません。
