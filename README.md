...
- **メインカラー2nd/3rd**：静止したUV0のレイヤーと色調補正を出力時に自動で焼き込みます。デカール、左右別、特殊アルファ、UV0以外などは自動ベイクできないため、先にlilToon側で焼き込んでください。既存VRMへの追加モードでは、未ベイクのマテリアルを検出したらワンクリック書き出しを案内します。
...
| FaceEmo | アバター内の保存済み設定から、登録済みグループ・表情パターンのデフォルト顔と分岐クリップを読み込みます。トリガーの端点は個別の表情になります。未登録パターン、VRChatの条件判定・連続入力・追跡制御の再現は対象外です |
