...
既定値や無条件の制御と同じ重みを再設定するだけの項目はポーズとして収集しません。Animation追跡設定は選択状態の親StateMachineからも読み取ります。
Buttonは押下後にパラメーターが0へ戻っても同じ状態を保つ構成が対象です。解除で別状態へ戻る構成は理由付きで除外します。体の動きの判定には部位マスクを適用し、書き出さない表情・材質・小物・頭のカーブは含めません。
筋肉/Transformの評価方式と合成可否も、マスク・重みを反映した有効なカーブとレイヤーで判定します。
