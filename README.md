...
VRChatのTracking Controlで体・手足・指を外部追跡へ戻す構成は、別レイヤーや以前の状態の設定も含めて理由付きで除外します。首・顔・目の追跡設定はポーズ対象外です。
メニュー条件は実際のAnimatorパラメーター宣言と照合します。未定義・重複・型に合わない条件・Triggerへの依存は未対応理由として表示します。
