...
public static class ExporterBehaviorTests
{
    ...
    private static void CheckMaterials()
    {
        ...
        Check(LilToonEmissionPolicy.HasHdrTextureEmission(material), "A different HDR image is detected even after baking changes main texture identity.");
        material.Properties["_EmissionBlend"] = 0f;
        Check(!LilToonEmissionPolicy.HasHdrTextureEmission(material), "Zero-blend HDR emission needs no suppression.");
        material.Properties["_EmissionBlend"] = 1f;
        material.Properties["_EmissionColor"] = new UnityEngine.Color(1f, 1f, 1f);
        Check(!LilToonEmissionPolicy.HasHdrTextureEmission(material), "Ordinary LDR texture emission remains supported.");
        material.Properties["_EmissionColor"] = new UnityEngine.Color(1.4f, 1.4f, 1.4f, 0f);
        Check(!LilToonEmissionPolicy.HasHdrTextureEmission(material), "Transparent emission contributes no color.");
        material.Properties["_EmissionColor"] = new UnityEngine.Color(1.4f, 1.4f, 1.4f);
        material.Properties["_UseEmission"] = 0f;
        Check(!LilToonEmissionPolicy.HasHdrTextureEmission(material), "Disabled HDR emission needs no suppression.");
        material.Properties["_UseEmission"] = 1f;
        material.Properties["_EmissionMap"] = null;
        Check(!LilToonEmissionPolicy.HasHdrTextureEmission(material), "Scalar-only HDR emission is outside the texture policy.");
        record = LilToonMaterialReader.Read(material, 0, (_, __) => 0);
