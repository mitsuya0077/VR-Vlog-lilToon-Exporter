...
namespace VRVlog.LilToonExporter
{
    ...
    internal static class VrChatGestureExpressions
    {
        internal static void Add(GameObject avatar, VrChatExpressionMenu.Source source)
        {
            ...
            foreach (var layer in controller.layers) Index(layer.stateMachine, layer.name);
            void MarkSubtree(AnimatorStateMachine machine)
            {
                foreach (var child in machine.states) targets.Add(child.state);
                foreach (var child in machine.stateMachines) MarkSubtree(child.stateMachine);
            }
            void Inspect(AnimatorTransitionBase transition)
            {
                if (transition.mute || !transition.conditions.Any(c => IsGesture(c.parameter))) return;
                if (transition.destinationState != null) targets.Add(transition.destinationState);
                if (transition.destinationStateMachine != null) MarkSubtree(transition.destinationStateMachine);
            }
