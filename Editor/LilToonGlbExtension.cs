...
namespace VRVlog.LilToonExporter
{
    internal static class LilToonGlbExtension
    {
        ...
        private static void RequireVrm10Root(Dictionary<string,object> root){var used=Array(root,"extensionsUsed",false);var required=Array(root,"extensionsRequired",false);var extensions=Object(root,"extensions",false);if(used==null||!Contains(used,"VRMC_vrm")||required==null||!Contains(required,"VRMC_vrm")||extensions==null||!extensions.TryGetValue("VRMC_vrm",out var raw)||!(raw is Dictionary<string,object> vrm)||!vrm.TryGetValue("specVersion",out var version)||!(version is string text)||text!="1.0"||!vrm.TryGetValue("humanoid",out var humanoid)||!(humanoid is Dictionary<string,object>)||!vrm.TryGetValue("meta",out var meta)||!(meta is Dictionary<string,object>))throw new InvalidOperationException("Fallback GLB has no valid required VRMC_vrm 1.0 root extension with humanoid and meta objects.");}
