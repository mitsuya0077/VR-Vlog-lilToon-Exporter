...
namespace VRVlog.LilToonExporter
{
    internal static class LilToonGlbExtension
    {
        ...
        private static void RequireVrm10Root(Dictionary<string,object> root)
        {
            ...
            foreach(var pair in bones){if(!(pair.Value is Dictionary<string,object> bone)||!bone.TryGetValue("node",out var rawNode)||!(rawNode is long node)||node<0||node>=nodeCount)throw new InvalidOperationException($"VRMC_vrm human bone '{pair.Key}' has no valid node.");if(!assignedNodes.Add(node))throw new InvalidOperationException($"VRMC_vrm human bone '{pair.Key}' reuses node {node}.");}
